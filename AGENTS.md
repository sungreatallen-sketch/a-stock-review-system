...
- 推荐执行窗口：T 日收盘后发布 → **T+1 开盘买入 → T+2 收盘卖出**
- 评估必须等 T+2 出现；任一执行价缺失保持 pending，不部分结算
- 北京时间交易日16:00后只自动推送当日终版，不跨日补发；手动晚间复盘保留；首次成功发送按报告交易日记录，重新生成不重置；日历见 config/trading_calendar_2026.json
- 所有修复/更新必须记日志（R29）
- 运行前预读规则（R30）
- 因子未验证未批准不得接入（R28）
