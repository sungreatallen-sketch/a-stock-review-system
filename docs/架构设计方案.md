...
## 8. 架构补充（v1.1，2026-09-02）

### 8.1 真实执行与结算链路
```
T 日收盘后发布推荐
  → T 日价仅作 reference_price
  → T+1 收盘价 = buy_price
  → T+2 收盘价 = sell_price
  → ret = T+2 close / T+1 close - 1
```
未到 T+2，或 T+1/T+2 任意收盘价缺失时保持 pending；禁止用 T 日参考价或未来数据伪造结果。

### 8.2 16:00 终版报告调度
```
launchd 16:00 / RunAtLoad / 30分钟轮询
  → 电源检查（仅插电运行）
  → 目标交易日 = 最近已收盘交易日
  → 同花顺 API + WorkBuddy MCP 健康检查
  → 无终版报告或只有盘中旧报告 → force 刷新
  → 校验 V01-V08
  → 飞书卡片 + 图片直发
```
终版判定必须同时满足：目标交易日收盘后生成、3 只推荐齐全、T 日收盘参考价齐全。`sent flag` 必须绑定 `report.meta.generated_at`。

### 8.3 数据源与部署约束
- 同花顺 API 是当前行情/K线主源；MCP 和 ego 按数据类型兜底。
- 本地 MCP 客户端强制绕过系统/VPN代理；只接受真实监听的本地聚合端口，不回落陈旧端口。
- launchd 使用 ASCII 入口 `/Users/yage/ashare-scripts/run_ashare_review_v15.sh`；项目通过 `/Users/yage/ashare-project` 符号链接访问，避免中文路径触发 launchd/zsh 解析问题。
